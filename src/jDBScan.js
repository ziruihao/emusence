...
// THIS CLUSTERING ALGO IS NOT MY CODE, IT IS FROM: https://github.com/upphiminn/jDBSCAN
...
const jDBSCAN = function () {
  ...
  function get_region_neighbours(point_idx) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      // console.log(distance(data[i], d), '<=', eps)

      if (
        time_eps &&
        time_distance(data[i], d) <= time_eps &&
  ...
  dbscan.getClusters = function () {
    ...
    for (let i = 0; i < num_clusters; i++) {
      ...
      if (distance === haversine_distance) {
        ...
        for (let j = 0; j < clusters[i].length; j++) {
          clusters_centers[i].location.latitude +=
            data[clusters[i][j]].location.latitude;
          clusters_centers[i].location.longitude +=
            data[clusters[i][j]].location.longitude;
          clusters_centers[i].location.accuracy +=
            data[clusters[i][j]].location.accuracy;
        }
      ...
      if (time_eps) {
        ...
        clusters_centers[i].duration.start = array_min(
          clusters[i],
          function (d) {
